...
const AiToolsMenu: React.FC<AiToolsMenuProps> = ({ settings }) => {
  ...
  return (
    <div className="fixed bottom-6 left-6 z-50 flex items-center">
      {/* Toggle Button */}
      <div 
        className="flex items-center gap-2 px-4 md:px-6 py-2 bg-[#202124]/80 backdrop-blur-xl border border-white/10 rounded-full cursor-pointer hover:bg-white/10 transition-all duration-300 z-20 shadow-lg"
        onClick={toggleMenu}
      >
      ...
      >
        ...
        <ToolLink href="https://www.perplexity.ai/" label="Perplexity" accentColor={settings.accentColor}>
            <svg xmlns="http://www.w3.org/2000/svg" height="100%" width="100%" viewBox="0 0 24 24">
                <circle style={{ fill: settings.accentColor }} cx="50%" cy="50%" r="50%"></circle>
                <path className="fill-white" style={{ transform: 'scale(0.8)', transformOrigin: 'center' }} d="m5.735 2l5.695 5.247V2.012h1.109v5.259L18.259 2v5.983h2.349v8.629h-2.342v5.327l-5.727-5.032v5.09h-1.11V16.99L5.742 22v-5.388H3.393v-8.63h2.342zm4.86 7.078H4.5v6.439h1.24v-2.031zM6.85 13.972v5.585l4.58-4.034V9.81zm5.72 1.497l4.588 4.03v-2.887h-.006v-2.646l-4.582-4.16zm5.696.048H19.5v-6.44h-6.047l4.814 4.363zm-1.115-7.534V4.519l-3.76 3.464zm-6.548 0l-3.76-3.464v3.464z"></path>
            </svg>
        </ToolLink>
        ...
        <ToolLink href="https://firefly.adobe.com/" label="Adobe Firefly" accentColor={settings.accentColor}>
            <svg height="100%" viewBox="0 0 24 24" width="100%" xmlns="http://www.w3.org/2000/svg">
                <rect style={{ fill: settings.accentColor }} width="100%" height="100%" rx="50%"></rect>
                <path className="fill-white" style={{ transform: 'scale(0.55)', transformOrigin: 'center' }} d="m21.512 6.098-1.849-.417a.741.741 0 0 0-.68.18l-1.39 1.285c-.548.507-1.424.008-1.26-.72l.417-1.85a.735.735 0 0 0-.18-.679l-1.285-1.391c-.507-.548 0-1.416.729-1.252l1.849.417c.237.058.499-.008.68-.18l1.39-1.284c.548-.508 1.424-.009 1.26.72l-.417 1.849c-.057.245.008.5.18.68l1.285 1.39c.507.548 0 1.416-.729 1.252zM5.76 23.738c0 .18-.081.262-.262.262h-3.74c-.146 0-.204-.106-.204-.262V4.402c0-.172.058-.237.23-.237H13.77c.205 0 .254.057.287.253l.335 3.265c.033.18-.057.262-.253.262H5.76v4.599h7.553c.172 0 .254.057.254.237v3.306c0 .18-.09.237-.237.237H5.76ZM16.635 9.207c0-.172.082-.254.254-.254h3.723c.197 0 .254.05.254.246v14.516c0 .197-.057.278-.278.278h-3.65c-.204 0-.303-.09-.303-.286z"></path>
            </svg>
        </ToolLink>
      </div>
    </div>
...
const ToolLink: React.FC<{ href: string; label: string; children: React.ReactNode; accentColor: string }> = ({ href, label, children, accentColor }) => (
  <a 
    href={href} 
    className="flex items-center gap-2 px-3 py-2 bg-[#202124]/80 backdrop-blur-xl border border-white/10 rounded-full hover:bg-white/10 transition-all duration-300 group"
  >
